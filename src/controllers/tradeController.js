  ...
  try {
    ...
    if (user) {
      const totalNetPnL = mergedCompletedTrades.reduce((sum, trade) => {
        ...
        const totalCharges = trade.exchangeRate + trade.brokerage;
        return sum + tradePnL - totalCharges;
      }, 0);
...
// Get trades by date with comprehensive summary
...
exports.updateTrade = async (req, res) => {
  try {
    ...
    res.status(400).send({
      error: error.message || "An error occurred while updating the trade",
    });
  }
};
...
exports.deleteTrade = async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(req.user._id).session(session);
    if (user) {
      await user.updateCapital(-pnLImpact, trade.date);
    }
...
exports.updateCompleteTrade = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const newPnL =
        (updatedTrade.sellingPrice - updatedTrade.buyingPrice) *
        updatedTrade.quantity;
      const pnLDifference = newPnL - oldPnL;
      await user.updateCapital(pnLDifference, updatedTrade.date);
    }
